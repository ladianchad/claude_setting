...
| Skill | 호출 | 설명 |
|-------|------|------|
| **coding** | `/coding` | 빌드/타입체크/테스트 자기 검증 + subagent 독립 리뷰 루프 |
| **debug** | `/debug` | 증상 정리 → 가설 수립 → 최소 검증 → git bisect 이분 탐색 |
| **design** | `/design` | 복수 subagent 독립 설계 → 합의안 도출 → 독립 검증 |
| **sprint** | `/sprint <파일>` | 요구사항 파일 기반 규모 판단 → 설계 → 구현 → 검증 원스톱 |

### 논문

| Skill | 호출 | 설명 |
|-------|------|------|
| **paper** | `/paper` | 다중 에이전트 아웃라인 합의 → 병렬 섹션 작성 → 전문가 패널 리뷰 |
| **paper_exam** | `/paper_exam` | PC Reviewer 3명 + 기술 전문가 + Area Chair의 모의 심사 |
| **paper_review** | `/paper_review <깊이>` | `요약` / `비평` / `풀리뷰` 3단계 깊이의 논문 분석 |
| **paper_submit** | `/paper_submit` | `/paper` + `/paper_exam` 체이닝, Accept까지 자동 반복 |
...
| `skills/*/SKILL.md` | 각 skill의 프로세스, 검증 기준 |
