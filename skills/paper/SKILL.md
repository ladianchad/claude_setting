...
매몰 방지는 escalation.md를 적용한다 (동일 이슈에 같은 접근 2회 실패 시 에스컬레이션).

**핵심 원칙**: 매 라운드마다 모든 리뷰어를 새 subagent로 생성한다. 이전 라운드의 리뷰 결과, 수정 내역, 컨텍스트를 전달하지 않는다. 매번 논문을 처음 보는 것처럼 판단한다.
...
모든 전문가를 **병렬 subagent**로 생성한다. 각 subagent에게 전달:
- 전체 논문 텍스트
...
### Step 3: Figure 전문가 리뷰 (별도 subagent)

Step 2와 병렬로 Figure 전문가 subagent를 생성한다. 전달: 전체 논문 텍스트 + Figure/Table 목록.
...
### Step 4: 학회/저널 심사위원 리뷰 (별도 subagent)

Step 2, 3과 병렬로 심사위원 subagent **2명**을 생성한다. 각 심사위원은 독립적으로 판단한다.
...
#### 5-2. 판정
- **Critical 또는 Major가 1개 이상**: 수정 후 다음 라운드로.
- **Minor만 남음**: Minor 수정 후 Phase 4로.
- **이슈 없음**: Phase 4로.

#### 5-3. 수정
- Critical/Major 이슈를 모두 수정한다.
- 수정 시 새로운 문제를 만들지 않도록 주변 맥락을 함께 점검.

#### 5-4. 매몰 방지 (escalation.md 적용)
- 동일 이슈가 2라운드 연속 Critical/Major로 지적되면:
  1. 같은 접근으로 3번째 수정을 시도하지 않는다.
  2. 새 subagent를 생성하여 문제를 위임한다:
     - 전달: 해당 섹션 텍스트 + 이슈 내용 + 이전 시도(사실만).
     - 전달 금지: 실패 원인 해석, 가설.
  3. subagent 방향이 다르면 우선 시도.
  4. subagent도 같은 결론이면 사용자에게 에스컬레이션:
     - 미해결 이슈 요약 + 시도한 접근 + 의심 원인.
     - 사용자 판단을 기다린다.

#### 5-5. 다음 라운드
- 수정 완료 후 **Step 1로 돌아간다**.
- Step 2~4의 모든 리뷰어는 새 subagent로 생성한다 (이전 라운드 결과 전달 금지).
...
3. **함께 제출**:
   - 전체 라운드별 리뷰 요약 및 수정 내역.
   - 잔여 이슈 (추가 실험 필요 등).
   - 자기 평가 (paper_review Verdict 기준).
   - Figure 전문가의 최종 시각 자료 권고 사항.
4. 사용자 피드백 시 해당 섹션 수정 → Phase 3 Step 1부터 재검증.
